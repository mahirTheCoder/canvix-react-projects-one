...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        id="Navbar "
        className=" hidden lg:block absolute top-0 left-0 w-full z-20"
      >
        <div className="container">
          <div className="navRow flex justify-between items-center pt-8  ">
              ...
              {navItems.map((item, k) => (
                <ul key={k}>
                  <li className="relative group">
                    <Link
                      key={k}
                      className="text-primary text-base font-dm "
                      to={item.navLink}
                    >
                      {item.navContent}
                    </Link>
                      ...
                      <div className="dropdown-manu absolute w-[250px] bg-white rounded-[7px] py-3 pl-2 top-8 invisible opacity-0 group-hover:visible group-hover:opacity-100  ">
                        ...
                        <ul>
                          {item.dropDown.map((dropwdwn, i) => (
                            <li key={i}>
                              <Link to={dropwdwn.dropDownLink}>
                                {dropwdwn.dropDownContent}
                              </Link>
                            </li>
                          ))}
                        </ul>
            ...
            <div className="navButton  flex justify-center items-center ">
              <button className=" w-[140px] h-[47px] rounded-[23px] bg-[#FBFBFB] text-secondary text-lg normal font-dm hover:bg-black hover:text-primary transition duration-700 inline-block">
                Get in touch
              </button>
            </div>
          </div>
